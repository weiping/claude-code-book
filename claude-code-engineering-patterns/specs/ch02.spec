spec: task
name: "第2章：Bootstrap 与全局状态——50+ 函数背后的单例设计"
tags: [book-chapter, part-1]
---

## 意图

揭示 `src/bootstrap/state.ts`（1762 行）为何选择模块级变量而非依赖注入作为全局状态管理方案，以及这一选择在 Agent 系统中的实际工程效果。读者读完后能说清"轻量全局状态"模式的问题场景、解决方案与取舍，并能在自己的项目中判断何时该用此模式、何时该用依赖注入。
...
- 源码引用格式：`src/相对路径:行号`
- 聚焦 src/bootstrap/state.ts，不展开调用方的业务逻辑
- 本章与第1章的关系：第1章说"全局状态模块存在"，本章说"它具体怎么设计"
...
- 不分析排除范围 stub 模块
- 不重复第1章对 bootstrap 模块的"存在性"介绍

## 完成条件

场景: hunter 风格开篇
  测试: ch02_hunter_opening
  假设 读者打开第2章
  当 读者阅读第一屏内容
  那么 ⛔ 开篇不直接引用任何源码路径或行号，而是以问题场景+模式预告+价值承诺三要素切入（150-200字）

场景: 模块级单例 vs 依赖注入的对比
  测试: ch02_singleton_vs_di
  假设 读者阅读核心设计节
  当 读者追问"为什么不用依赖注入"
  那么 章节有明确的选 A 不选 B 的权衡说明，并附源码证据

场景: 源码引用有效性
  测试: ch02_source_anchors
  层级: 集成
  假设 读者跟随章节中的源码引用
  当 读者在项目目录查找每处路径和行号
  那么 每处引用都实际存在于对应文件
...
场景: 函数数量具体化
  测试: ch02_function_count
  假设 本章提到 bootstrap/state.ts 的函数数量
  当 读者验证数字
  那么 使用精确数字（通过 grep 统计实际 export function 数量）而非"50+"泛指

场景: 模式命名框存在
  测试: ch02_pattern_box
  假设 本章使用 hunter 风格
  当 读者检查章末
  那么 存在至少 1 个格式规范的模式命名框，命名可移植的工程模式

场景: 章末行动建议
  测试: ch02_action_items
  假设 读者读完本章
  当 读者检查"你能做什么"节
  那么 包含 5-8 条以行动动词开头的可操作建议

场景: 写入并发安全问题有说明
  测试: ch02_concurrency_note
  假设 模块级变量在并发场景下存在风险
  当 读者检查本章对并发安全的处理
  那么 章节明确说明此模式在单线程（Bun 主线程）环境下的前提假设

场景: 不引用排除范围
  测试: ch02_no_excluded_refs
  层级: 集成
  假设 DESIGN.md 列出了排除范围
  当 读者检查本章所有源码路径
  那么 没有路径落在 src/assistant/、src/ssh/、src/server/、src/proactive/ 中


场景: ⛔ hunter 开篇格式（最高优先级）
  测试: ch02_hunter_opening_format
  假设 读者打开本章
  当 读者阅读第一屏内容（开篇节）
  那么 ⛔ 开篇不直接引用任何源码路径或行号
...
场景: 适用范围表存在
  测试: ch02_applicability_table
  假设 本章提炼了一个工程模式
  当 读者检查"适用范围"节
  那么 存在一个表格，列出该模式适用（✓）和不适用（✗）的场景，每行附理由和替代方案

场景: 权衡与局限分析
  测试: ch02_tradeoffs_and_limits
  假设 本章提炼了一个工程模式
  当 读者检查"权衡与局限"节
  那么 章节说明了该模式的适用边界、潜在失败风险、性能影响和替代方案

场景: 与已知模式的对话
  测试: ch02_known_pattern_dialogue
  假设 本章提炼了一个工程模式
  当 读者检查"与已知模式的对话"节
...
- bootstrap/state.ts 的所有调用方业务逻辑（只分析状态模块本身）
- 依赖注入框架的详细实现（只做概念对比，不展开框架源码）
