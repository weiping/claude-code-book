spec: task
name: "第 22 章：AutoCompact 与上下文折叠——窗口压缩的触发机制"
tags: [book-chapter, part-6]
---

## 意图

揭示 `src/services/compact/autoCompact.ts`（351行）和 `src/utils/analyzeContext.ts` 中 `AUTOCOMPACT_BUFFER_TOKENS` 如何检测上下文窗口临界、自动触发压缩，以及 `contextCollapse` 在保证语义连续性的同时缩减 token 用量的实现原理。读者读完后能理解"上下文窗口管理"模式，并能在自己的长对话 Agent 中实现类似的自动压缩机制。
...
- 源码引用格式：`src/相对路径:行号`
- 核心文件：src/services/compact/autoCompact.ts、src/utils/analyzeContext.ts、src/services/contextCollapse/
- 前置依赖：第 19 章（系统提示组装）、第 21 章（会话持久化）
...
场景: hunter 风格开篇
  测试: ch18_hunter_opening
  假设 读者打开第 22 章
  当 读者阅读第一屏内容
  那么 ⛔ 开篇不直接引用任何源码路径或行号，而是以问题场景+模式预告+价值承诺三要素切入（150-200字）

场景: 触发临界值具体化
  测试: ch18_threshold_value
  假设 读者阅读触发条件节
  当 读者检查临界值的具体数字
  那么 章节使用精确的 token 数字（如 AUTOCOMPACT_BUFFER_TOKENS 的实际值），而非"接近上限时"

场景: 语义连续性保证说明
  测试: ch18_semantic_continuity
  假设 读者阅读压缩策略节
  当 读者追问"压缩后 AI 是否还知道之前发生了什么"
  那么 章节展示 contextCollapse 如何在压缩时保留关键摘要信息

场景: 源码引用有效性
  测试: ch18_source_anchors
  层级: 集成
  假设 读者跟随章节中的源码引用
  当 读者在项目目录查找每处路径和行号
  那么 每处引用都实际存在于对应文件
...
场景: 模式命名框存在
  ...
  假设 本章使用 hunter 风格
  当 读者检查章末
  那么 存在至少 1 个格式规范的模式命名框，提炼"自适应上下文压缩"模式

场景: 章末行动建议
  测试: ch18_action_items
  假设 读者读完本章
  当 读者检查"你能做什么"节
  那么 包含 5-8 条以行动动词开头的可操作建议

场景: 不引用排除范围
  测试: ch18_no_excluded_refs
  层级: 集成
...
场景: 适用范围表存在
  测试: ch18_applicability_table
  假设 本章提炼了一个工程模式
  当 读者检查"适用范围"节
  那么 存在一个表格，列出该模式适用（✓）和不适用（✗）的场景，每行附理由和替代方案

场景: 权衡与局限分析
  测试: ch18_tradeoffs_and_limits
  假设 本章提炼了一个工程模式
  当 读者检查"权衡与局限"节
  那么 章节说明了该模式的适用边界、潜在失败风险、性能影响和替代方案

场景: 与已知模式的对话
  测试: ch18_known_pattern_dialogue
  假设 本章提炼了一个工程模式
  当 读者检查"与已知模式的对话"节
