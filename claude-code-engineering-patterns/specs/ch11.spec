spec: task
name: "第11章：权限决策树——PermissionMode 的四层设计"
...
- 前置依赖：第10章（Tool 接口中的 isAllowed 字段）
...
- 不展开 AI 分类器（第12章）
- 不展开第33章的全局安全架构（本章只讲 PermissionMode 层）
...
- AI 分类器 autoModeState（第12章）
- 第33章的全局安全架构（三层防线）
- PermissionRule 规则引擎细节（第34章）
