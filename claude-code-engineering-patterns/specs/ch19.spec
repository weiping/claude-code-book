spec: task
name: "第 19 章：提示词装配——fetchSystemPromptParts() 的分层组装"
...
- 前置依赖：第 8 章（QueryEngine 调用 fetchSystemPromptParts）
...
### 允许修改
- book/src/part6/ch19.md

### 禁止做
- 不修改 DESIGN.md 或其他章节文件
- 不展开 CLAUDE.md 层级扫描细节（第 20 章）
- 不展开记忆系统三层架构（第 23 章）
