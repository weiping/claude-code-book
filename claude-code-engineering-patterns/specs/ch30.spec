spec: task
name: "第 30 章：LocalAgentTask——子 Agent 的进程模型与 I/O 协议"
tags: [book-chapter, part-8]
---

## 意图

揭示 `src/tasks/LocalAgentTask/LocalAgentTask.tsx`（639行）如何通过子进程隔离运行独立的 Agent，以及父子 Agent 之间结构化 I/O 协议的设计——输入如何传入、输出如何回传、错误如何传播。读者读完后能理解子进程 Agent 的进程模型，并能在自己的多 Agent 系统中设计类似的 I/O 协议。
...
- 源码引用格式：`src/相对路径:行号`
- 核心文件：src/tasks/LocalAgentTask/LocalAgentTask.tsx（639行）
- 前置依赖：第 29 章（Task 系统总体架构）
...
- 不重复第 29 章对 Task 类型体系的描述
- 不展开 Swarm 的多 Agent 协作（第29-32章）

## 完成条件

场景: hunter 风格开篇
  测试: ch26_hunter_opening
  假设 读者打开第 30 章
  当 读者阅读第一屏内容
  那么 ⛔ 开篇不直接引用任何源码路径或行号，而是以问题场景+模式预告+价值承诺三要素切入（150-200字）

场景: 进程模型图存在
  测试: ch26_process_model_diagram
  假设 读者阅读进程模型节
  当 读者检查图表
  那么 包含一个图表，展示父进程与子 Agent 进程的关系和通信通道

场景: I/O 协议说明
  测试: ch26_io_protocol
  假设 读者阅读 I/O 协议节
  当 读者检查父子进程的数据交换
  那么 章节展示输入传入（stdin/参数）、输出回传（stdout/结构化消息）的具体格式

场景: 源码引用有效性
  测试: ch26_source_anchors
  层级: 集成
  假设 读者跟随章节中的源码引用
  当 读者在项目目录查找每处路径和行号
  那么 每处引用都实际存在于对应文件
...
场景: 模式命名框存在
  ...
  假设 本章使用 hunter 风格
  当 读者检查章末
  那么 存在至少 1 个格式规范的模式命名框，提炼"子进程 Agent 隔离"模式

场景: 章末行动建议
  测试: ch26_action_items
  假设 读者读完本章
  当 读者检查"你能做什么"节
  那么 包含 5-8 条以行动动词开头的可操作建议

场景: 不引用排除范围
  测试: ch26_no_excluded_refs
  层级: 集成
...
场景: 适用范围表存在
  测试: ch26_applicability_table
  假设 本章提炼了一个工程模式
  当 读者检查"适用范围"节
  那么 存在一个表格，列出该模式适用（✓）和不适用（✗）的场景，每行附理由和替代方案

场景: 权衡与局限分析
  测试: ch26_tradeoffs_and_limits
  假设 本章提炼了一个工程模式
  当 读者检查"权衡与局限"节
  那么 章节说明了该模式的适用边界、潜在失败风险、性能影响和替代方案

场景: 与已知模式的对话
  测试: ch26_known_pattern_dialogue
  假设 本章提炼了一个工程模式
  当 读者检查"与已知模式的对话"节
...
- Swarm 多智能体协作架构（第29-32章）
- RemoteAgentTask（第 31 章）
- DreamTask（第 32 章）
