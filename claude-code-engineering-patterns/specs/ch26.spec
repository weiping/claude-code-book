spec: task
name: "第 26 章：AsyncHookRegistry——异步钩子的注册、超时与并发控制"
tags: [book-chapter, part-7]
---

## 意图

揭示 `src/utils/hooks/AsyncHookRegistry.ts`（309行）如何处理异步钩子的完整生命周期：挂起、超时、取消、结果回调，以及为何需要一个专门的注册表而非直接 await。读者读完后能将这一注册表模式用于自己的异步钩子系统，理解"挂起→完成/超时/取消"三态的工程价值。
...
- 源码引用格式：`src/相对路径:行号`
- 核心文件：src/utils/hooks/AsyncHookRegistry.ts（309行）
- 前置依赖：第 24 章（Hook 事件体系）、第 25 章（执行器触发异步钩子）
...
场景: hunter 风格开篇
  测试: ch22_hunter_opening
  假设 读者打开第 26 章
  当 读者阅读第一屏内容
  那么 ⛔ 开篇不直接引用任何源码路径或行号，而是以问题场景+模式预告+价值承诺三要素切入（150-200字）

场景: 三态生命周期说明
  测试: ch22_three_state_lifecycle
  假设 读者阅读生命周期节
  当 读者检查异步钩子的状态描述
  那么 章节明确说明挂起、完成、超时/取消三种状态的转换条件，并有状态图

场景: 超时机制具体化
  测试: ch22_timeout_mechanism
  假设 读者阅读超时节
  当 读者检查超时的具体处理
  那么 章节展示超时阈值的来源（配置/硬编码）和超时后的处理逻辑

场景: 源码引用有效性
  测试: ch22_source_anchors
  层级: 集成
  假设 读者跟随章节中的源码引用
  当 读者在项目目录查找每处路径和行号
  那么 每处引用都实际存在于对应文件
...
场景: 模式命名框存在
  ...
  假设 本章使用 hunter 风格
  当 读者检查章末
  那么 存在至少 1 个格式规范的模式命名框，提炼"异步钩子注册表"模式

场景: 章末行动建议
  测试: ch22_action_items
  假设 读者读完本章
  当 读者检查"你能做什么"节
  那么 包含 5-8 条以行动动词开头的可操作建议

场景: 不引用排除范围
  测试: ch22_no_excluded_refs
  层级: 集成
...
场景: 适用范围表存在
  测试: ch22_applicability_table
  假设 本章提炼了一个工程模式
  当 读者检查"适用范围"节
  那么 存在一个表格，列出该模式适用（✓）和不适用（✗）的场景，每行附理由和替代方案

场景: 权衡与局限分析
  测试: ch22_tradeoffs_and_limits
  假设 本章提炼了一个工程模式
  当 读者检查"权衡与局限"节
  那么 章节说明了该模式的适用边界、潜在失败风险、性能影响和替代方案

场景: 与已知模式的对话
  测试: ch22_known_pattern_dialogue
  假设 本章提炼了一个工程模式
  当 读者检查"与已知模式的对话"节
