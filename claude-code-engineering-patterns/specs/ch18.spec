spec: task
name: "第 18 章：Skill 系统——声明式能力包的加载与执行"
...
- 前置依赖：第 14 章（Tool 接口，SkillTool 实现同一接口）
...
### 允许修改
- book/src/part5/ch18.md

### 禁止做
- 不修改 DESIGN.md 或其他章节文件
- 不展开第 28 章的 Frontmatter Hooks 注册（本章只讲 Skill 加载与执行，不讲 Hook 注册）
- 不重复第 14 章对 buildTool 的描述
