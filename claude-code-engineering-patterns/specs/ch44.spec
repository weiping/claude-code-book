spec: task
name: "第 44 章：Harness 工程模式图谱——12 个可复用设计模式"
tags: [book-chapter, part-12]
---

## 意图

从前 39 章的源码分析中提炼出 12 个可复用的 Agent Harness 工程模式，每个模式有名称、问题陈述、解决方案和源码锚点，可直接用于读者自己的 Agent 系统设计。这是全书的模式汇总章节，不引入新的源码分析，只做模式提炼与系统化。
...
- 源码引用格式：`src/相对路径:行号`（引用前章已验证的锚点）
- 前置依赖：第1-39章（所有模式均来自前章的源码分析结论）
- 模式数量：12 个（DESIGN.md 确认值）
...
### 允许修改
- book/src/part12/ch44.md

### 禁止做
- 不修改 DESIGN.md 或其他章节文件
- 不引入前39章未分析过的新源码
- 不重复前章的详细源码追踪（用"详见第 N 章"代替）

## 完成条件

场景: hunter 风格开篇
  测试: ch40_hunter_opening
  假设 读者打开第44章
  当 读者阅读第一屏内容
  那么 ⛔ 开篇不直接引用任何源码路径或行号，而是以问题场景+模式预告+价值承诺三要素切入（150-200字）

场景: ⛔ hunter 开篇格式（最高优先级）
  测试: ch40_hunter_opening_format
  假设 读者打开本章
  当 读者阅读第一屏内容（开篇节）
  那么 ⛔ 开篇不直接引用任何源码路径或行号
...
场景: 适用范围表存在
  测试: ch40_applicability_table
  假设 本章提炼了一个工程模式
  当 读者检查"适用范围"节
  那么 存在一个表格，列出该模式适用（✓）和不适用（✗）的场景，每行附理由和替代方案

场景: 权衡与局限分析
  测试: ch40_tradeoffs_and_limits
  假设 本章提炼了一个工程模式
  当 读者检查"权衡与局限"节
  那么 章节说明了该模式的适用边界、潜在失败风险、性能影响和替代方案

场景: 与已知模式的对话
  测试: ch40_known_pattern_dialogue
  假设 本章提炼了一个工程模式
  当 读者检查"与已知模式的对话"节
