# 第 31 章：RemoteAgentTask——远程任务的通信架构

> "不透明的远端不需要透明的协议，只需要足够可靠的探针。"

---

本地运行的 Agent 任务（第 30 章的 LocalAgentTask）和远程运行的 Agent 任务从 UI 角度看起来完全一样——都显示在任务面板里，都有进度更新，都在完成时通知用户。但底层实现截然不同：本地任务通过 React 状态直接驱动，远程任务需要定期向云端服务器"探针"——用轮询同步一份本地镜像状态。
...
远程任务代理解决了"本地如何感知远端"的问题。DreamTask 则是另一个方向——它不是追踪已在运行的任务，而是在用户不在时主动发起后台自主执行，这是第 32 章的主题（详见第 32 章）。
