...
**图 9-1：三重困境与 Async Generator 的解法对应**
...
**图 9-3：SSE 原始事件 → queryModel → 结构化消息的三段管道**
...
- **循环层**（`queryLoop`）：多轮工具调用编排，背压自然控制（详见第 8 章）
...
流式响应管道将 SSE 字节流转化为结构化事件序列，最终这些事件以工具调用的形式驱动 `queryLoop` 继续迭代（详见第 8 章）。而工具的接口契约——每个工具必须实现什么接口、工具调用如何被编排执行——将在第 10 章展开。
