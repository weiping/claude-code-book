# 第 4 章：用户输入分流——`processUserInput` 的路由决策树

> "在副作用执行之前就完成分流，是这套路由的设计宣言。"
> （原文："Runs before attachment extraction so this path matches the slash-command path below."）

在 Claude Code 的代码库中，有一个函数在所有消息处理路径上只出现一次，但它的结构在整个系统中被反复复用：**先判断输入类型，再懒加载处理器，期间不触碰任何业务状态**。这就是本章要讲的「前置路由（Pre-dispatch Router）」模式——代码库中至少有 4 条路径遵循这一结构。

识别这个模式，你就能在自己的 Agent 系统中构建可测试、无副作用的多类型输入分流器。
...
**图 4-2：static import vs dynamic import 的架构差异**
...
| 需要运行时动态注册新的输入类型 | ✗ | if-chain 是静态的，添加新类型需改代码 | 插件注册表（详见第 37 章）|
...
下一章（第 5 章）将深入斜杠命令系统的内部——当 `processSlashCommand` 收到一条以 `/` 开头的输入时，103 个命令如何注册、加载和执行。
