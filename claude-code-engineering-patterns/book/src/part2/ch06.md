# 第 6 章：模型自动选择——Opus、Sonnet 与 Haiku 的路由逻辑
...
Effort 和 thinking 机制会进一步影响模型的使用深度——同一个模型在不同 effort 级别下的行为差异，以及 thinking 预算如何控制 Opus 的推理深度，将在第 7 章详细展开。本章的三层路由是"选择哪个模型"，第 7 章解决"如何用好这个模型"。
