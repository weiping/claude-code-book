# 第 33 章：Teammate 生命周期——Swarm 架构总览
...
**图 33-1：Leader-Teammate Swarm 架构**
...
Teammate 的初始化协议建立了角色关系。但初始化之后，当 Teammate 遇到需要权限决策的操作时，它怎么向 Leader "请示"？这是 `leaderPermissionBridge` 的职责——第 34 章将深入这个跨进程权限协商机制（详见第 34 章）。
