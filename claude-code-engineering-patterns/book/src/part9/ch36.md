# 第 36 章：permissionSync——多智能体间的权限同步协议
...
- **in-process Teammate**：直接通过 `leaderPermissionBridge`（第 34 章）调用 Leader 的 React 状态更新
...
**图 36-2：SwarmPermissionRequest 状态机**
...
**前置条件**：共享文件系统（同机器）；有 mailbox 基础设施（第 37 章）；权限请求频率低（文件 IO 开销可接受）。
...
permissionSync 解决了多 Agent 之间权限请求的传递协议。而当 Leader 收到这些请求后，如何在拦截→规则匹配→用户确认三个层次中决定是批准还是拒绝，这是第 37 章权限系统全景的主题（详见第 37 章）。
