# 第 39 章：Plugin 包结构——目录约定、`package.json` 契约与沙箱边界
...
**图 39-2：PluginManifestSchema 的能力组合结构**
...
第 39 章揭示了插件包的目录约定、清单覆盖机制，以及沙箱边界的两个环境变量。第 40 章和第 41 章将从更高视角回望全书：提炼 12 个可复用的 Harness 工程模式，并展望从 CLI 到 Agent 平台的架构跃迁路线图（详见第 40 章）。
