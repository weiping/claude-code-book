# 第 19 章：提示词装配——fetchSystemPromptParts() 的分层组装
...
**图 19-1：三层并行组装。** 注意 customSystemPrompt 分支的双重清空——自定义提示不是追加，而是完全替换默认提示和系统上下文。
...
**图 19-2：customSystemPrompt 的双重清空逻辑**
...
**图 19-2：customSystemPrompt 的双重清空。** 用户上下文（CLAUDE.md）是唯一在覆盖模式下仍然生效的层——体现了对用户配置声明的尊重。
...
**图 19-3：模式的四个组成要素。** 每个要素解决一个特定问题：层定义解决顺序语义，并行拉取解决延迟，结构化返回解决可组合性，覆盖语义解决优先级冲突。
...
**图 19-5：两条路径的差异。** 主路径额外注入 coordinator 和记忆模式 prompt；fallback 路径接受可能的 cache miss，但保证功能可用。
...
**图 19-4：三种模式的对比。** 每种模式都解释了设计的某个维度，但都不能完整描述——分层提示词组装是三种思想的交汇点，而非任何一种的直接实例。
