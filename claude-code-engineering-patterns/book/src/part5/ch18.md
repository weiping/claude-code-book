...
**图 18-1：两种模型的安全边界。** 插件模型的用户代码直接进入主进程，声明式能力包的执行路径（fork 子进程）与主进程完全隔离。
...
**图 18-2：frontmatter 到 Tool 的完整转化链路**
...
**图 18-2：Skill 从文件到执行的完整路径。** 注意正文内容（体积大）在延迟加载路径上，只在实际调用时才进入上下文。
...
**图 18-3：三种执行路径。** 远程技能最轻量（内容注入），fork 技能最隔离（独立 Agent），普通技能处于中间（内联展开）。
...
**图 18-4：三层结构与执行路径。** 声明层和内容层各自独立，执行层负责组合——这是关注点分离在 AI 能力扩展上的具体体现。
...
**图 18-5：与三种业界模式的关系定位。** 每种已知模式都能解释声明式能力包的某个维度，但没有一种完整覆盖——文件系统发现、AI 内容执行、frontmatter 剥离的组合是这个模式的独特之处。
