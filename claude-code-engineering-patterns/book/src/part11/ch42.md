# 第 42 章：Marketplace 协议——官方注册表与第三方源
...
第 42 章揭示了 Marketplace 协议的多源设计——discriminated union 统一协议类型，单一分发函数隔离协议细节，官方来源通过 `sourceIsFallback` 实现透明注入。Marketplace 被加载后，其中的插件包遵循什么样的目录约定和 `package.json` 契约？这是第 43 章的主题（详见第 43 章）。
