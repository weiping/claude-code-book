# 第 28 章：声明式钩子注册——从 CLAUDE.md 到执行器的绑定链路
...
声明式钩子注册是 Hooks 系统的用户界面层。Hooks 系统全景的另一端——权限决策如何拦截工具调用、规则引擎如何评估——将在接下来的第 29 章和第 38 章展开（详见第 29 章）。
