# 第 30 章：leaderPermissionBridge——跨进程权限协商模式
...
**图 30-1：leaderPermissionBridge 注册时序**
...
`leaderPermissionBridge` 解决的是进程内 Teammate 与 Leader 之间的权限协商。当多个 Teammate 同时对同一个权限做决策时——不同 Teammate 的权限决策如何合并？这是 `permissionSync` 的职责，也是第 32 章的主题（详见第 32 章）。第 31 章将先介绍三种 Teammate 后端的选型逻辑（详见第 31 章）。
