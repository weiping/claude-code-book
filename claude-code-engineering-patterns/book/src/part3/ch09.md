# 第 9 章：QueryEngine 主循环——工具调用的编排逻辑
...
**图 9-3：主 Agent 与子 Agent 超轮次的不同生命周期**
...
**与 GoF 命令模式（Command Pattern）的比较**。命令模式把操作封装为对象，由调用方决定何时执行。`Tool` 接口（详见第 14 章）确实是一种命令封装——工具就是一个可执行的命令对象。**但命令模式中，执行结果不会反馈给命令发起者并影响下一个命令的选择。** 在多轮工具调用循环中，每次工具执行结果都会被 AI 读取，AI 据此决定下一步调用什么——**执行结果形成了一个反馈环（feedback loop）**，这是传统命令模式中不存在的结构。
