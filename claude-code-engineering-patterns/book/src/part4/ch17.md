...
**图 17-2：消息类型过滤——isTranscriptMessage 的守门逻辑**
...
**潜在失败风险**：`recordTranscript` 的 `await` 意味着如果磁盘写入本身失败（磁盘满、权限错误），会抛出异常并中断整个消息处理流程——用户会看到错误，而不是静默丢失数据。这是"失败关闭"（fail-closed）语义，符合 Agent 系统对可靠性的要求（详见第 11 章对权限系统失败关闭设计的分析）。
...
会话持久化的下一层是**上下文折叠**（AutoCompact）：当 transcript 增长到接近上下文窗口上限时，Claude Code 如何在保持语义连续性的同时压缩历史？这个触发机制建立在本章介绍的 `sessionStorage` 之上，详见第 18 章。记忆系统的三层架构（本地记忆、跨会话提取、团队共享）则构成了更高层的持久化语义，详见第 19 章。
