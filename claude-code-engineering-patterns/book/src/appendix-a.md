...
本附录列出 Claude Code 支持的全部 27 个 Hook 事件，供快速查阅。每个事件包含触发时机、输入格式、退出码语义和典型用途。完整的钩子系统设计分析见第 20-22 章。
...
**matcher 字段**：`PreToolUse/PostToolUse/PostToolUseFailure/PermissionDenied/PermissionRequest` 支持按 `tool_name` 过滤，只对特定工具触发。
...
| 事件名 | 触发时机 | 输入 JSON 字段 | 退出码语义 | 典型用途 |
|--------|---------|--------------|----------|---------|
| `SessionStart` | 新会话开始时 | `source`（会话来源）| 0=stdout 传给 Claude；其他=仅报错给用户 | 注入初始上下文、配置检查 |
| `SessionEnd` | 会话即将结束时 | `reason`（结束原因）| 0=正常完成；其他=报错给用户 | 会话摘要、清理工作 |
| `Stop` | Claude 即将结束响应前 | —（无特定字段）| 0=不显示；2=报错给模型并继续对话；其他=仅报错给用户 | 响应后处理 |
| `StopFailure` | 因 API 错误（限流/认证失败等）结束时 | `error` | Fire-and-forget：钩子输出和退出码均被忽略 | 错误监控（不可阻断）|
| `Setup` | 仓库初始化和维护时 | `trigger`（`init` 或 `maintenance`）| 0=stdout 传给 Claude；其他=仅报错给用户 | 项目初始化脚本、依赖检查 |
...
| `Notification` | 系统发送通知时 | `message`, `notification_type` | 0=不显示；其他=仅报错给用户 | 自定义通知转发（如发到 Slack）|
| `PermissionDenied` | （见工具调用类）| — | — | — |
...
| 事件名 | 触发时机 | 输入 JSON 字段 | 退出码语义 | 典型用途 |
|--------|---------|--------------|----------|---------|
| `PreCompact` | 对话压缩前 | 压缩详情（含 `trigger`）| 0=stdout 追加为自定义压缩指令；2=阻止压缩；其他=仅报错给用户 | 自定义压缩摘要策略 |
...
| 事件名 | 触发时机 | 输入 JSON 字段 | 退出码语义 | 典型用途 |
|--------|---------|--------------|----------|---------|
| `SubagentStart` | Agent 工具调用启动子 Agent 时 | `agent_id`, `agent_type` | 0=stdout 传给子 Agent；其他=仅报错给用户 | 子 Agent 上下文注入 |
| `SubagentStop` | 子 Agent 完成后 | `agent_id`, `agent_type`, `agent_transcript_path` | 0=不显示；2=报错给子 Agent 继续运行；其他=仅报错给用户 | 子 Agent 结果审计 |
...
| 事件名 | 触发时机 | 输入 JSON 字段 | 退出码语义 | 典型用途 |
|--------|---------|--------------|----------|---------|
| `Elicitation` | MCP 服务器请求用户输入时 | `mcp_server_name`, `message`, `requested_schema` | 0=使用钩子响应；2=拒绝请求；其他=仅报错给用户 | 自动回填 MCP 输入 |
...
| 事件名 | 触发时机 | 输入 JSON 字段 | 退出码语义 | 典型用途 |
|--------|---------|--------------|----------|---------|
| `ConfigChange` | 会话期间配置文件变更时 | `source`（配置来源）, `file_path` | 0=允许变更；2=阻止变更应用到会话；其他=仅报错给用户 | 配置变更审计 |
| `InstructionsLoaded` | 加载 CLAUDE.md 或规则文件时 | `file_path`, `memory_type`, `load_reason`, `globs`, `trigger_file_path`, `parent_file_path` | 0=正常；其他=仅报错给用户 | **仅观测，不可阻断** |
...
| `CwdChanged` | 工作目录切换后 | `old_cwd`, `new_cwd` | 0=正常；其他=仅报错给用户 | 目录切换时更新环境变量（写入 `CLAUDE_ENV_FILE`）|
| `FileChanged` | 被监听的文件变更时 | `file_path` | 0=正常；其他=仅报错给用户 | 自动触发重新分析 |
...
| 其他（1, 非0）| 报错，但通常不阻断 | stderr 仅展示给用户，操作继续 |

**注**：`StopFailure` 和 `InstructionsLoaded` 是 Fire-and-forget 事件，忽略退出码。

---

*基于 `src/utils/hooks/hooksConfigManager.ts` v2.1.88 提取，共 27 个事件。*
