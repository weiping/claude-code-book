...
这不是一个 CLI。这是一套 **Agent Harness**。
 ...
 Harness，是一个工程术语，指的是将不可控的外部系统包裹在可控接口之内的框架结构。Claude Code 把 Anthropic 的 Claude 模型变成一个可编排的 Agent 运行时——它的每一个子系统（Tool、Hook、Task、Swarm、Context Engineering）都在做同一件事：**在 AI 的不确定性和工程系统的确定性之间，修一道桥梁**。
...
如果你正在设计自己的 Agent 系统，或者在评估"到底应该用框架还是自己造轮子"，这本书提供了一个 50 万行级别生产代码的真实样本。如果你想理解现代 AI 应用背后的工程基础设施如何层层叠加，这本书的 10 篇结构从输入到渲染逐层剥开这个谜题。如果你只是好奇——序言之后的 41 章随时欢迎你从任意一章跳进去。

阅读建议：

- **按顺序读**：第二篇的第 4 章开始是完整的查询引擎追踪线，适合想从头理解一次交互全貌的读者
- **按图索骥**：如果你对某个子系统特别感兴趣——Hook（第 20 章）、Task（第 25 章）、Swarm（第 29 章）——每章都是独立入口
- **模式优先**：第十篇（第 40 章）的模式图谱适合作为复习清单，在读完其他章节后回头对照
...
- **第1章：系统总体架构与技术栈——Claude Code 的结构地图**
  一句话主张：读完这章，你将建立 Claude Code 的完整架构认知——7 大子系统的职责边界、技术选型（Bun/TypeScript/Ink/OpenTelemetry）的设计动机，以及 2,000 个源码文件的组织逻辑。
  （前置依赖：无）
...
*目的：按"输入→分流→命令→查询→流式响应"的真实数据流，完整追踪一次交互的生命周期*

- **第4章：用户输入分流——`processUserInput` 的路由决策树**
  一句话主张：读完这章，你将理解 Claude Code 如何在收到用户输入后的第一毫秒内完成分流——斜杠命令、普通消息、特殊模式三条路径的判断逻辑，以及"路由决策先于任何业务逻辑"这一架构原则。
  （前置依赖：第2章）

- **第5章：斜杠命令系统——103 个命令的注册、加载与执行**
  一句话主张：读完这章，你将理解 Claude Code 的命令系统如何通过目录约定 + 动态加载 + feature flag 门控实现 103 个命令的一致性管理，并能将这一"目录即注册表"模式用于自己的可扩展 CLI 设计。
  （前置依赖：第4章）

- **第6章：模型自动选择——Opus、Sonnet 与 Haiku 的路由逻辑**
  一句话主张：读完这章，你将理解 Claude Code 如何根据任务复杂度动态路由到 Haiku（高频轻量）、Sonnet（主力开发）、Opus（深度推理）三个模型，以及 `mainLoopModelOverride` 和 `setModelForTask` 如何实现模型选择与覆盖机制。
  （前置依赖：第4章）

- **第7章：Effort、Fast Mode 与 Thinking——推理深度控制的三轴**
  一句话主张：读完这章，你将理解 Claude Code 如何通过 effort 级别（小时级任务）、fast mode（低延迟优先）、thinking 模式（内部推理链）三个正交维度控制 AI 的推理深度，以及这三者如何与模型选择交叉作用。
  （前置依赖：第6章）

- **第8章：QueryEngine 主循环——工具调用的编排逻辑**
  一句话主张：读完这章，你将理解 Claude Code 如何把"用户输入 → AI 响应 → 工具调用 → 再次请求"变成一个稳定的主循环，并能识别其中的重试与中断模式。
  （前置依赖：第4章）

- **第9章：流式响应管道——异步生成器的工程应用**
  一句话主张：读完这章，你将理解 `query.ts` 如何用 async generator 把 SSE 流转化为结构化事件序列，以及这一模式在高延迟 AI 调用中的价值。
  （前置依赖：第8章）

---

### 第三篇：Tool 系统——可扩展的工具协议
*目的：理解工具如何被定义、注册、权限校验、执行*

- **第10章：Tool 接口契约——`buildTool()` 工厂与 Zod 输入验证**
  一句话主张：读完这章，你将理解 Claude Code 的工具系统如何通过统一工厂函数实现 40+ 工具的一致性，并掌握"接口契约 + 运行时验证"的工具注册模式。
  （前置依赖：第2章）

- **第11章：权限决策树——PermissionMode 的四层设计**
  一句话主张：读完这章，你将理解 `PermissionMode` 如何把"默认拒绝 → 规则允许 → 用户确认 → 策略覆盖"组织成可组合的决策链，并能将其用于自己的 Agent 权限系统。
  （前置依赖：第10章）

- **第12章：AI 分类器替代规则引擎——yolo-classifier 的设计哲学**
  一句话主张：读完这章，你将理解为什么 Claude Code 用 AI 模型而非规则集来判断"这个操作是否安全"，以及这一决策的工程成本与收益。
  （前置依赖：第11章）

- **第13章：MCP 工具协议——把第三方工具变成一等公民**
  一句话主张：读完这章，你将理解 Model Context Protocol 如何让外部服务注册为 Claude Code 工具，并能分析协议化扩展与插件化扩展的架构差异。
  （前置依赖：第10章）
...
*目的：理解 Claude Code 如何精确控制送入 AI 的每一个 token——从提示词组装到记忆管理到窗口压缩*

- **第15章：提示词装配——`fetchSystemPromptParts()` 的分层组装**
  一句话主张：读完这章，你将理解每次 API 调用前系统提示是如何被动态构建的——工具声明、记忆注入、CLAUDE.md 内容、用户上下文的组装顺序与优先级规则。
  （前置依赖：第8章）

- **第16章：CLAUDE.md 注入——层级化指令的加载与优先级**
  一句话主张：读完这章，你将理解 `memdir` 系统如何从当前目录向上扫描 CLAUDE.md 文件、合并多层级指令，以及层级优先级规则如何在团队协作场景中防止指令冲突。
  （前置依赖：第15章）

- **第17章：会话持久化——转录、快照与断点续写**
  一句话主张：读完这章，你将理解 `sessionStorage` 如何在进程重启后恢复会话状态，并能识别"写入先于执行"这一持久化模式在 Agent 系统中的可靠性保证价值。
  （前置依赖：第8章）

- **第18章：AutoCompact 与上下文折叠——窗口压缩的触发机制**
  一句话主张：读完这章，你将理解 Claude Code 如何检测上下文窗口临界、自动触发压缩、维护 compact 边界标记，以及 `contextCollapse` 在保证语义连续性的同时缩减 token 用量的实现原理。
  （前置依赖：第15章、第17章）

- **第19章：记忆系统——本地、跨会话与团队记忆的三层架构**
  一句话主张：读完这章，你将理解 Claude Code 如何通过 `memdir`（本地记忆）、`extractMemories/SessionMemory`（跨会话提取）、`teamMemorySync`（团队共享）三层架构，让 AI 在不同范围内积累和共享知识，并能将这一分层记忆模式用于自己的 Agent 设计。
  （前置依赖：第16章）

---

### 第五篇：Hooks 引擎——Harness 的核心
*目的：深入理解 Hook 系统——这是全书最重要的部分*

- **第20章：20+ 事件的生命周期地图——Hooks 全景**
  一句话主张：读完这章，你将能从 `PreToolUse` 到 `SubagentStop` 完整描述一个 AI 操作的钩子触发序列，并理解每个事件的设计意图。
  （前置依赖：第10章）

- **第21章：四种执行器——`command/prompt/agent/http` 的多态设计**
  一句话主张：读完这章，你将理解 Claude Code 如何用多态执行器把"运行 shell 命令"、"调用 AI"、"发 HTTP 请求"统一在同一个 Hook 接口下，并能识别开放-封闭原则在此的体现。
  （前置依赖：第20章）

- **第22章：AsyncHookRegistry——异步钩子的注册、超时与并发控制**
  一句话主张：读完这章，你将理解 `AsyncHookRegistry` 如何处理钩子的异步生命周期——挂起、超时、取消、结果回调——并能将这一注册表模式用于自己的异步钩子系统。
  （前置依赖：第20章）

- **第23章：hooksConfigSnapshot——配置快照隔离模式**
  一句话主张：读完这章，你将理解为什么 Claude Code 在会话启动时"冻结"Hook 配置而非实时读取，以及这一快照隔离模式如何防止配置变更影响正在运行的会话。
  （前置依赖：第20章）

- **第24章：Frontmatter Hooks——Skill 与 Agent 的声明式钩子注册**
...
*目的：理解 Claude Code 如何管理并发的后台任务*

- **第25章：7 种任务类型的状态机——Task 系统设计**
...
| 章 | 篇 | 一句话主张 |
|----|----|-----------|
| 第1章 | 一 | 建立 Claude Code 完整架构认知：7 大子系统职责、技术选型动机、源码组织逻辑 |
| 第2章 | 一 | 掌握"轻量全局状态"模式：模块级单例 vs 依赖注入的工程权衡 |
| 第3章 | 一 | 用构建时 DCE 替代运行时 if/else 实现零成本特性隔离 |
| 第4章 | 二 | 理解"路由决策先于业务逻辑"原则：三条分流路径的判断逻辑 |
| 第5章 | 二 | 掌握"目录即注册表"模式：103 个命令的动态加载与 feature flag 门控 |
| 第6章 | 二 | 理解 Haiku/Sonnet/Opus 三层模型路由与 mainLoopModelOverride 覆盖机制 |
| 第7章 | 二 | 理解 effort/fast/thinking 三轴如何交叉控制 AI 推理深度 |
| 第8章 | 二 | 识别 AI 工具调用主循环的编排模式与重试/中断设计 |
| 第9章 | 二 | 掌握 async generator 在高延迟 AI 流式响应中的工程应用 |
| 第10章 | 三 | 掌握统一工厂函数 + 运行时验证的工具注册模式 |
| 第11章 | 三 | 理解四层权限决策链并将其用于自己的 Agent 权限系统 |
| 第12章 | 三 | 分析用 AI 分类器替代规则引擎的工程成本与收益 |
| 第13章 | 三 | 对比协议化扩展（MCP）与插件化扩展的架构差异 |
| 第14章 | 三 | 理解"声明式能力包"模式：Skill frontmatter 到可执行工具的完整转化链路 |
| 第15章 | 四 | 掌握系统提示分层组装的顺序与优先级规则 |
| 第16章 | 四 | 理解 CLAUDE.md 多层级扫描与指令合并的防冲突机制 |
| 第17章 | 四 | 理解"写入先于执行"持久化模式与跨进程会话恢复 |
| 第18章 | 四 | 掌握上下文窗口压缩的触发时机与语义连续性保证 |
| 第19章 | 四 | 理解本地/跨会话/团队三层记忆架构的边界与同步机制 |
| 第20章 | 五 | 完整描述一个 AI 操作从 PreToolUse 到 SubagentStop 的钩子序列 |
| 第21章 | 五 | 识别多态执行器模式在 Hook 系统中的开放-封闭原则体现 |
| 第22章 | 五 | 掌握异步钩子注册表的挂起、超时、取消、回调完整生命周期 |
| 第23章 | 五 | 理解配置快照隔离如何保证会话内行为一致性 |
| 第24章 | 五 | 分析声明式钩子注册 vs 命令式注册的工程权衡 |
| 第25章 | 六 | 理解 7 种任务类型的统一状态机与并发安全的类型守卫 |
| 第26章 | 六 | 掌握子进程 Agent 的 I/O 协议设计 |
| 第27章 | 六 | 分析远程任务中 SSE/WebSocket/轮询三种通信方式的选型逻辑 |
| 第28章 | 六 | 理解"Agent 主动触发"与"用户触发"任务的架构本质差异 |
| 第29章 | 七 | 理解 Leader/Teammate 多智能体模型与资源清理机制 |
| 第30章 | 七 | 掌握跨进程权限代理模式的实现机制 |
| 第31章 | 七 | 分析 in-process/tmux/auto 三种后端的性能与隔离性权衡 |
| 第32章 | 七 | 理解并发 Agent 场景中权限状态的一致性保证机制 |
| 第33章 | 八 | 描述三层防线的整体协同架构及任意层失守时的降级策略 |
| 第34章 | 八 | 掌握 allowedTools 规则的三种匹配算法与多来源冲突优先级解决机制 |
| 第35章 | 八 | 理解同一权限请求在 interactive/coordinator/swarmWorker 三态下的分叉逻辑 |
| 第36章 | 八 | 掌握用户决策持久化、跨轮次拒绝追踪与安全审计机制 |
| 第37章 | 九 | 理解插件从发现到卸载的完整生命周期及状态管理机制 |
| 第38章 | 九 | 掌握 Marketplace 协议支持官方注册表和第三方 npm 源的扩展机制 |
| 第39章 | 九 | 理解插件包的内部结构、依赖隔离与进程沙箱边界 |
| 第40章 | 十 | 获得可直接使用的 12 个 Agent Harness 工程模式图谱 |
| 第41章 | 十 | 能为自己的产品绘制从 CLI 到多智能体平台的演化路线图 |
...
7. **`src/bridge/` 完整实现**：Bridge/远程控制协议的完整分析（仅在 Task/Swarm 章节中必要时引用）
...
- **关键类型**：`Tool`、`Task`、`TaskType`、`TaskStatus`、`HookEvent`、`HookCommand`、`PermissionMode`、`PermissionResult`、`ToolUseContext`、`AppState`

### 附录 D：环境变量参考
- **内容**：Claude Code 所有支持的环境变量速查
- **列**：变量名 | 类型 | 默认值 | 说明 | 影响的模块
- **关键变量**：`ANTHROPIC_API_KEY`、`CLAUDE_MODEL`、`CLAUDE_API_BASE`、`HTTP_PROXY`、`NO_COLOR`、`CLAUDE_CFG_HOOKS_PATH`
...
- **关键 Flags**：`KAIROS`（Assistant 模块）、`SONNET`（模型相关）、`COMPUTER_USE`（计算机使用）
- **来源**：`src/main.tsx` + `bun:bundle` 使用处

### 附录 F：术语表
- **内容**：全书使用的技术术语与专有名词定义
- **格式**：术语（英文原文）| 章节语境 | 一句话定义 | 关联术语
- **收录范围**：Harness、Reconciler、Yoga Layout、Yoga Node、ANSI Escape Sequence、CSI/OSC/DEC/SGR、Feature Flag、DCE（Dead Code Elimination）、PermissionMode、HookEvent、Swarm、Teammate、Ink、termio、SGR、Cursor Managed 等
